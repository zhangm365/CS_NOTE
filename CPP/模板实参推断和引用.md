...
`&&` 可能是左值引用，也可能是右值引用，因此称为万能引用（`universal reference`）。

### 5.1 万能引用的场合

如果一个变量或参数声明为 `T&&`，其中 `T` 是被推导的类型，那么这个变量或参数就是一个万能引用。
...
int a = 1;
f(a);    // 传入左值，那么 T&& 就是 lvalue reference, 即左值引用绑定了左值
f(1);    // 传入右值，那么 T&& 就是 rvalue reference, 即右值引用绑定了右值
```
