...
int main(int argc, char **argv)
{
	...
	if(argc != 3)
	{
		printf("info : %s src_file des_file\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	struct stat *src_info = malloc(sizeof(struct stat)); //malloc
	struct stat *des_info = malloc(sizeof(struct stat));
	stat(argv[1], src_info);
	stat(argv[2], des_info);
	
	
	// 2, throw tasks
	if( S_ISREG(src_info->st_mode) && S_ISREG(des_info->st_mode) )
	{
		int fd[2];
	...
	else if( (S_ISDIR(src_info->st_mode) && S_ISDIR(des_info->st_mode)) || (S_ISREG(src_info->st_mode) && S_ISDIR(des_info->st_mode)) )
	{
